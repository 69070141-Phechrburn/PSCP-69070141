...
```text
input จำนวนร้านค้าทั้งหมดและจำนวนช่วงเวลาที่ต้องการตรวจสอบ
input ต่อมาคือ บอกเวลาเปิดและปิดของร้านค้านั้นๆ (เป็นนาที)
input สุดท้ายคือ รายชื่อนาทีที่ต้องการเช็คว่ามีร้านเปิดอยู่กี่ร้าน
output ตัวเลขจำนวนร้านค้าที่เปิดอยู่ ณ นาทีที่ถูกถาม โดยแสดงผลลัพธ์ทุกตัวเรียงกันใน บรรทัดเดียว คั่นด้วยช่องว่าง
```
...
```text
สร้าง Array/List ชื่อ open_stores ขนาด 1,441 ช่อง (Index 0 ถึง 1440) กำหนดค่าเริ่มต้นเป็น 0 ทุกช่อง

การบันทึกข้อมูลร้านค้า (Input & Update):
รับค่าจำนวนร้าน (num) และจำนวนคำถาม (check)
วนลูปตามจำนวนร้าน num รอบ:
รับค่าเวลาเปิด (start) และเวลาปิด (stop)
วนลูปตั้งแต่นาทีที่ start จนถึง stop - 1
บวกร้านเพิ่มที่ open_stores[minute] += 1

การดึงคำตอบ (Query & Output):
รับค่ารายการเวลาที่ต้องการเช็คทั้งหมด (check_times)
วนลูปอ่านค่านาที k จากรายการคำถาม
ดึงจำนวนร้านจาก open_stores[k] มาเก็บใส่ลิสต์คำตอบ

แสดงผลลัพธ์ทั้งหมดออกมาในบรรทัดเดียวกัน คั่นด้วยช่องว่าง
```
